...
LL solve(int ty) {
	...
	FOR(i,0,n-1) FOR(j,i,n-1) {
		...
		if (len <= max_length){
			...
			int MR = min((j == n-1 ? (::n-1) : tmp[j+1]-1), tmp[i] + max_length - 1);
			
			int P1 = MR-max_length+1;
			int P2 = tmp[j] - max_length + 1;
			LL tail = (MR - tmp[j] + 1);
			LL add = 0;
			if (ML >= P1)
				add = tail * (tmp[i] - ML + 1);
				...
				int y = MR - tmp[j] + 1;
				add = tail * (tmp[i] - P1 + 1) + sum(y - 1) - sum(y - (P1 - ML + 1));
			}
			res += add;
			//cout << tmp[i] << " " << tmp[j] << " " << add << endl;
		}
	}
	//cout << res << endl;
	return res;
